<?php
	...
	function get_contact($var, $specific = '') {
		...
		if ( $var == 'all' or $var == 'count' ) {
			$q = $db->prepare ( "SELECT * FROM vos_contact $specific" );
		}		
		elseif ( is_email($var) ) {
			$q = $db->prepare ( "SELECT * FROM vos_contact WHERE email = ? ORDER BY date DESC" );
			$q->bind_param ( 's', $var );
		}
		elseif( is_numeric($var) ) {
			$q = $db->prepare ( "SELECT * FROM vos_contact WHERE id_contact = ?" );
			$q->bind_param ( 'i', $var );
		}
	...
	function delete_contact($i) {
		...
		$q = $db->prepare ( "DELETE FROM vos_contact WHERE id_contact = ?" );
		$q->bind_param ( 'i', $i );
	...
	function update_contact ($i, $table, $value) {
		...
		$q = $db->prepare ( "UPDATE vos_contact SET $table = ? WHERE id_contact = ?" );
	...
	function new_contact($name, $email, $comments, $phone = '') {
		global $db;

		if ( empty($name) or !is_email($email) or empty($comments) ) {
			return false;
		}
		...
		$q = $db->prepare ( "INSERT INTO vos_contact (name, phone, email, comments, status, status_user, date, date_responded, visitor_ip, visitor_city, visitor_state, visitor_country, visitor_browser, visitor_OS, visitor_referer, visitor_page) 
							VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)" );
