<?php
	...
	function get_group_setting() {
		...
		$q = $db->prepare ( "SELECT cat, group_name FROM vos_settings WHERE cat != 0 ORDER BY cat ASC" );
		$q->execute();
		$result = $q->get_result();
	...
	function get_setting($var, $specific = '') {
		...
		if ( $var == 'all' ) {
			$q = $db->prepare ( "SELECT * FROM vos_settings $specific" );
			if ( !empty($_GET['p']) ) {
				$q->bind_param ( 's', $_GET['p'] );	
			}
		}
		elseif ( $specific == true ) {
			$q = $db->prepare ( "SELECT * FROM vos_settings WHERE id_setting = ?" );
			$q->bind_param ( 'i', $var );
		}
		else {
			$q = $db->prepare ( "SELECT value FROM vos_settings WHERE id_setting = ?" );
			$q->bind_param ( 'i', $var );
		}
	...
	function update_setting($i, $v) {
		...
		$q = $db->prepare ( "UPDATE vos_settings SET value = ? WHERE id_setting = ?" );
		$q->bind_param ( 'ss', $v, $i );		
