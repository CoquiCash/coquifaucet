...
class visitorTracking
{
    ...
    */
	...
	protected function track()
	{
		...
		//Prepare variables
		if ( !empty( $this->getIP(TRUE) ) ) { $visitor_ip = $this->getIP(TRUE) ; } else { $visitor_ip = ''; }

		//modification was done because of empty arrays errors
		if ( !empty( $this->geoCheckIP($this->getIP()) ) ) { $ip_location = $this->geoCheckIP($this->getIP()) ; } else { $ip_location = ''; }
		if ( !empty( $ip_location['town'] ) ) { $visitor_city = $ip_location['town']; } else { $visitor_city = ''; }
		if ( !empty( $ip_location['state'] ) ) { $visitor_state = $ip_location['state']; } else { $visitor_state = ''; }
		if ( !empty( $ip_location['country'] ) ) { $visitor_country = explode('-', $ip_location['country']); } else { $visitor_country = ''; }
		if ( !empty( $visitor_country[0] ) ) { $visitor_ccode = trim($visitor_country[0]); } else { $visitor_ccode = ''; }
		if ( !empty( $visitor_country[1] ) ) { $visitor_cname = trim($visitor_country[1]); } else { $visitor_cname = ''; }
		
		if ( !empty( $visitor_ccode ) ) { $visitor_flag = $this->getFlag($visitor_ccode); } else { $visitor_flag = ''; }
