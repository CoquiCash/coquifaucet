<?php
	...
	function send_contact_email ($from_name, $from_email, $comments) {
		global $translate;
		
		$link = get_domain()."/admin/contact.php?id=".get_contact($from_email)['id_contact'];
		
		$message = $translate->__("Hello").":<br /><br /><b>$from_name</b> ";
		$message = $message.$translate->__("Asked for coquis")." <b>".get_setting(1)."</b>.<br /><br />";
		$message = $message.$translate->__("The provided email is").": <b>$from_email</b> <br /><br />";
		$message = $message.$translate->__("The wallet address provided is").": <br /><br />".$comments."<br /><br />";
		$message = $message.$translate->__("To see more information, click the following link").": <br /><br /><a href='$link' target='_blank'>$link</a>";	

		$subject = $translate->__("New Request Notification of")." $from_name";
